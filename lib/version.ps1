...
<#
    ...
    $ruby, $gem = get_shim_execution $PSCommandPath
    & $ruby $gem $args
#>
#
# Note: <2023-02-11> This function is not used anymore!
# But is reserved for later check
...
# Function:
#   Added in <2023-02-11> for shim to get the gem path
#   For shim to use
function shim_get_gem_name($cmd_path) {
    # $PSCommandPath must have a : to represent drive
    # E.g.
    # C:Ruby-on-Windows\shims\bin\cr.ps1
    $f = fname $cmd_path # Now 'cr.ps1'
    $cmd = strip_ext $f  # Now 'cr'
    $gem = "$env:RBENV_ROOT\shims\bin\$cmd"
